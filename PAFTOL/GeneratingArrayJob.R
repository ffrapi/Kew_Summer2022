...
script <- args[12] # name of the master script in the docker
...
config_file <- data.frame(ArrayTaskID = 1:n_alltasks, env_files=env_files, libs=selected_libraries)
write_tsv(config_file, paste0(input_path, "/", slurm_job, ".config"))
sink(paste0(input_path, "/", slurm_job, "_array_job.sh"))
cat("#!/bin/bash\n\n#SBATCH --job-name=\"", slurm_job,
    ...
    "\n# Extract the environment file name for the current $SLURM_ARRAY_TASK_ID\nenvfile=$(awk -v ArrayTaskID=$SLURM_ARRAY_TASK_ID '$1==ArrayTaskID {print $2}' $config)\n\nlib=$(awk -v ArrayTaskID=$SLURM_ARRAY_TASK_ID '$1==ArrayTaskID {$        "\ncd ", output_path, "; zip -r ${lib}.zip ${lib}\nmv ", output_path, "/${lib}.zip $SCRATCH/private/2024JanuaryBatch/Output/", sep ="")
sink()
